...
function diffUsingJS() {
    ...
    diffoutputdiv.appendChild(diffview.buildView({
        ...
        // 0 : side by side; 1 : inline
        viewType: $("inline").checked ? 1 : 0,
        // if the line contains string "create time", then take it as equal
        ignoreLine: function(change, baseTextLine, newTextLine){
            if(change == "replace" && baseTextLine.indexOf("create time")>=0 &&  newTextLine.indexOf("create time")>=0){
                return true;
            }
        }
    }));
